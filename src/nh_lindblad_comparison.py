...
for n in n_pars:
    ...
    phi = 0
    comm = make_command(n_particles=n, group_name="nh/nh_lindblad_cmp_superposition", initial_state="superposition", theta=theta, phi=phi, nh=True)
    print(f"running simulation: {n=}, superposition")
    subprocess.run(comm, check=True)
