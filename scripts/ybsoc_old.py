...
class YbSOCSystem:
            ...
            if display_progress \
                ...
                for spin_up_state, spin_down_state in itertools.product(spin_up_states, spin_down_states):
                    ...
                    for q_idx in self.sites_multi_indices:
                        if umklapp:
                            momentum_1_idx_f = lattice.add_multi_indices(momentum_1_idx_i, q_idx, self.lengths)
                            momentum_2_idx_f = lattice.subtract_multi_indices(momentum_2_idx_i, q_idx, self.lengths)
                        else:
                            _momentum_1_idx_i = lattice.fold_bz(momentum_1_idx_i, self.lengths)
                            _momentum_2_idx_i = lattice.fold_bz(momentum_2_idx_i, self.lengths)
                            _q_idx = lattice.fold_bz(q_idx, self.lengths)
                            
                            momentum_1_idx_f = lattice.add_multi_indices(_momentum_1_idx_i, _q_idx)
                            momentum_2_idx_f = lattice.subtract_multi_indices(_momentum_2_idx_i, _q_idx)
                            if not (lattice.check_constrain(momentum_1_idx_f, self.lengths) and 
                                    lattice.check_constrain(momentum_2_idx_f, self.lengths)):
                                continue
                        ...
                        hamiltonian[new_idx, state_index] += parity * (-self.U) / (self.d * self.num_sites)
    ...
    def sparse_hamiltonian_scipy_csr(
        ...
        if kinetic:
            for state_index, multi_particle_state in enumerate(tqdm.tqdm(self.multi_particle_states)):
                iterator = iter(range(self.n_particle))
            ...
            for spin_up_state, spin_down_state in itertools.product(spin_up_states, spin_down_states):
                ...
                for q_idx in self.sites_multi_indices:
                    if umklapp:
                        ...
                        momentum_2_idx_f = lattice.subtract_multi_indices(_momentum_2_idx_i, _q_idx)
                        if not (lattice.check_constrain(momentum_1_idx_f, self.lengths) and 
                                lattice.check_constrain(momentum_2_idx_f, self.lengths)): # ignore 
                            continue
                        momentum_1_idx_f = lattice.fold(momentum_1_idx_f, self.lengths)
                        momentum_2_idx_f = lattice.fold(momentum_2_idx_f, self.lengths)
...
class YbSOC2bodyLoss(YbSOCSystem):
    def __init__(
        ...
        super().__init__(
            d, lengths, n_particle, hbar, q, m_Yb, delta, omega_R, 1j * gamma / 2, array_type
        )
        self._gamma = gamma
