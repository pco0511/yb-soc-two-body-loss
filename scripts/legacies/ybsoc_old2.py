...
class YbSOCSystem:
    ...
    def kinetic(self, q_x):
        H = np.array(
            [
                [
                    (self.hbar**2) * ((q_x - self.k_r) ** 2) / (2 * self.m_Yb)
                    + self.delta / 2,
                    self.omega_R / 2,
                ],
                [
        ...
        for state_index, multi_particle_state in enumerate(self.multi_particle_states):
            ...
            while i < N:
                ...
                if (i + 1 < N) and (momentum_idx == multi_particle_state[i + 1][0]):
                    ...
                    val = ((self.hbar**2) / (2 * self.m_Yb)) * (
                        (q_x - self.k_r) ** 2 + (q_x + self.k_r) ** 2
                    )
                    data.append(val)
    ...
    def dense_hamiltonian(self, kinetic: bool = True, interaction: bool = True):
    ...
    def sparse_hamiltonian_scipy_csr(
        self, kinetic: bool = True, interaction: bool = True
    ):
        data = []
        rows = []
        cols = []
