...
for idx, unfixed_params in tqdm.tqdm(enumerate(itertools.product(*metadata["unfixed"].values())), total=data_len):
    ...
    # eigenvalues, eigenvectors = jax.lax.linalg.eigh(hamiltonian)
    # lefteigenvectors = eigenvectors
    # righteigenvectors = eigenvectors
